...
- C语言中的位运算
 ...
 ![image.png](https://cdn.nlark.com/yuque/0/2022/png/2172986/1657459188772-3c4373b4-8858-4dcd-aff3-32717f7e8dab.png#averageHue=%23f7f7f3&clientId=ue6e3f7ab-5b23-4&crop=0&crop=0&crop=1&crop=1&from=paste&height=141&id=u8203327d&margin=%5Bobject%20Object%5D&name=image.png&originHeight=282&originWidth=1246&originalType=binary&ratio=1&rotation=0&showTitle=false&size=305941&status=done&style=none&taskId=u7a6f203b-b9eb-4d00-a74c-fe4e4a54324&title=&width=623)
   ...
   - 位运算的一个常见用法就是实现**掩码运算**，比如$x=0x89ABCDEF$，如果要得到最低有效字节的值，可以使用运算x&0xFF得到。
- C语言中的逻辑运算
...
- **无符号数编码**
   - 定义向量：$\overrightarrow x = [x_{w-1},x_{w-1}...,x_{0}]$，使用 **B2U**(Binary to Unsinged) 来表示无符号数编码：$B2U_w(\overrightarrow x) = \sum_{i=0}^{w-1} x_i2^{i}$
...
- **常规二进制小数表示**: 
   ...
   - 例如：$101.11(2) = 1*2^2+0*2^1+1*2^0+1*2^{-1}+1*2^{-2} = 15/4$点左边的位的权是2的正幂，点右边的位的权是2的负幂
   - 某些十进制小数，是无法用二进制小数表示的，比如1/5，只能通过扩大二进制小数的位数，无限接近1/5。
- **IEEE 浮点表示**: 
   - IEEE 浮点标准用$V=(-1)^s*M*2^E$来表示一个一个浮点数。
...
- IEEE浮点表示$V=(-1)^s*M*2^E$和 上图的二进制位计算关系：
   - **规格化值：**
...
![](https://cdn.nlark.com/yuque/0/2022/jpeg/2172986/1669520635123-a67e6c3c-edc9-49e8-a6a9-9c00c08b3dd8.jpeg)
      ...
      - **阶码**字段的值$E=exp-Bias$。$Bias=2^{k-1}-1$**。**本例中，exp有8位，所以$Bias=2^{8-1}-1$。**exp**即不全是0，也就不全是1。即exp的取值范围 (0,255)。
      - **尾数**:小数字段**frac**被描述为小数值$f$。$0\leq f<1$。$M=1+f,(f=2^{k-1}+2^{k-2}...)$
   - **非规格化值**:

![](https://cdn.nlark.com/yuque/0/2022/jpeg/2172986/1669521971622-ef42c0b4-e92e-425e-818e-6e77f96555a5.jpeg)

      - **符号**取决于S。可选0或1。决定了正数或负数。
      - **阶码**域全为0时，所标识的浮点数是**非规格化**。在这种情况下，阶码值是$E=1-Bias，Bias=2^{8-1}-1$
      - **尾数**：尾数值$M=f，0\leq f<1，M=1+f,(f=2^{k-1}+2^{k-2}...)$。
      - 示例：
         - 如果$s=0,frac=M=f=0,$代表$+0$，如果$s=1,farc=M=f=0$,代表$-0$。
         - 如果$frac\neq0$可以代表一个接近于0的数，取值范围$(0<result<1 || -1<result<0)$
   - **特殊值**:
      - **阶码**域全为1的时候，代表了特殊值。
...
- **浮点运算**:
   - $(3.14+1e10)-1e10 = 0.0$，表达式对$(3.14+1e10)$进行计算时,不会遵循加法结合律，会对结果进行舍入，3.14会丢失。得到结果0。
